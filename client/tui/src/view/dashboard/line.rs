...
pub(crate) struct DashboardLineIterator<'a> {
    cells: &'a [DashboardCell],
    view_size: Size,
}

impl<'a> DashboardLineIterator<'a> {
    pub fn new(cells: &'a [DashboardCell], view_size: Size) -> Self {
...
impl<'a> Iterator for DashboardLineIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while let Some(next) = self.cells.get(take) {
            let width = column_count(next.width);
        ...
        let subset: &'a [DashboardCell] = &self.cells[..take];
...
impl<'a> ratatui::widgets::Widget for &DashboardLine<'a> {
        ...
        Self: Sized,
    {
        ...
        let segments = Layout::horizontal(
            self.cells
                .iter()
                .map(|cell| Constraint::Ratio(column_count(cell.width) as u32, total)),
        )
        .split(area);
        for (area, cell) in segments.into_iter().zip(self.cells) {
            DashboardLineCell::from(cell).render(*area, buf);
        }
    }
}
